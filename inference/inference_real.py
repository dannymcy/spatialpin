...
def inference(
  ...
  for i, img_path in enumerate(data_path):
    ...
    # Add saving point cloud file here, issue involved
    for k, (pose, pc) in enumerate(zip(abs_pose_outputs, masked_pointclouds)):
        if (pose is None) or (pc is None):
          continue
        print("pc", pc.shape)
    
        # pc (point clouds) is numpy.ndarray, pose is simnet.lib.transform.Pose. Need to convert pc to tensor
        # print("pc type", type(pc))
        # print("pose type", type(pose))
        pc = torch.from_numpy(pc).to('cpu')

        pose = pose.camera_T_object @ (pose.scale_matrix)
        # pose is numpy.ndarray here. Need to convert pc to tensor
        pose = torch.from_numpy(pose).to('cpu')
        ...
        filename = 'masked_depth_pcd_' + str(i) + "_pose_" + str(k)+'.pcd'
    ...
    # Add saving projection visualization here
    o3d.visualization.draw_geometries(rotated_pcds)
    img_proj_rgb = cv2.cvtColor(img_vis, cv2.COLOR_BGR2RGB)
    save_projected_points(img_proj_rgb, points_2d, str(output_path), i)
