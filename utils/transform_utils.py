...
# deprecated
# https://www.open3d.org/docs/0.16.0/tutorial/geometry/file_io.html
def load_glb_to_ply_pcd(glb_path, obj_path, ply_path, pcd_path, pose):
    # mesh = o3d.io.read_triangle_mesh(glb_path)
    # tex_img = o3d.io.read_image('/hdd2/chenyang/shapo/results/ShAPO_Real/one2345_plus/can_0.png')
    # mesh.textures = [o3d.cuda.pybind.geometry.Image(tex_img)]
    # o3d.io.write_triangle_mesh(obj_path, mesh)

    obj_trimesh = trimesh.load(glb_path, force='mesh')
    obj_trimesh.apply_transform(pose.scale_matrix)
    obj_trimesh.apply_transform(pose.camera_T_object)
    obj_trimesh.export(ply_path)
    obj_trimesh = obj_trimesh.as_open3d
    obj_trimesh.compute_vertex_normals()

    # Projected textures
    single_pcd = obj_trimesh.sample_points_uniformly(200000)
    # print(np.array(single_pcd.points))
    # pred_rgb = get_rgb_from_rgbnet(emb, np.array(single_pcd.points), appearance_emb, rgbnet)

    rotated_pc, _, _ = get_pc_absposes(pose, np.array(single_pcd.points))
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(np.array(rotated_pc))
    # pcd.points = o3d.utility.Vector3dVector(np.array(single_pcd.points))
    # print(np.asarray(pcd.points))
    # pcd.colors = o3d.utility.Vector3dVector(np.array())
    pcd.normals = o3d.utility.Vector3dVector(np.array(single_pcd.normals))
    o3d.io.write_point_cloud(pcd_path, pcd)
    return obj_trimesh, pcd
